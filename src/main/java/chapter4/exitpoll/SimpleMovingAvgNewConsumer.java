        ...
        .addShutdownHook(
            new Thread() {
              ...
              public void run() {
                ...
                // Note that shutdownhook runs in a separate thread, so the only thing we can safely
                // do to a consumer is wake it up
    ...
    try {
      ...
      while (true) {
        ...
        for (TopicPartition topicPartition : movingAvg.consumer.assignment()) {
          System.out.println(
              "Committing offset at position: " + movingAvg.consumer.position(topicPartition));
        }
